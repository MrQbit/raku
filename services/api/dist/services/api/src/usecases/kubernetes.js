...
export async function generateKubernetesManifests(deploymentId, config = {}) {
    ...
    const { serverName, namespace, imageUrl, landingZone } = deployment;
    const { replicas = 2, resources = {
        requests: { cpu: "100m", memory: "128Mi" },
        limits: { cpu: "500m", memory: "512Mi" }
    }, service = { type: "ClusterIP", port: 80, targetPort: 3000 }, healthCheck = { enabled: true, path: "/health", initialDelaySeconds: 30, periodSeconds: 10 } } = config;
...
export async function deployToKubernetes(deploymentId) {
    const deployment = await prisma.mcpServerDeployment.findUnique({
        where: { id: deploymentId },
        include: {
            landingZone: true
...
export async function getDeploymentStatus(deploymentId) {
    ...
    try {
        ...
        const { stdout: deploymentStatus } = await execAsync(`kubectl get deployment ${deployment.serverName} -n ${deployment.namespace} -o json`);
        const deploymentData = JSON.parse(deploymentStatus);
        const status = deploymentData.status;
        // Get pods
        const { stdout: podsStatus } = await execAsync(`kubectl get pods -l app=raku-mcp,server=${deployment.serverName} -n ${deployment.namespace} -o json`);
        ...
        return {
            status: status.conditions?.find((c) => c.type === "Available")?.status === "True" ? "running" : "deploying",
...
export async function deleteKubernetesDeployment(deploymentId) {
    const deployment = await prisma.mcpServerDeployment.findUnique({
        where: { id: deploymentId },
        include: {
            landingZone: true

...
export async function generatePackageJson(config) {
...
export async function generateServerCode(config) {
    const { serverName, packs, port = 3000 } = config;
    // Generate intents from all packs
    const allIntents = packs.flatMap(pack => pack.intents.map(intent => ({
...
export async function buildDockerImage(config) {

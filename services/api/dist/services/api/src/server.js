...
app.get("/v1/servers/:id", async (req, reply) => reply.send(await getServer(req.params.id)));
app.get("/v1/packs", async (_req, reply) => reply.send(await listPacks()));
app.get("/v1/packs/:id", async (req, reply) => reply.send(await getPack(req.params.id)));
app.post("/v1/packs", async (req, reply) => reply.code(201).send(await createPack(req.body)));
app.put("/v1/packs/:id", async (req, reply) => reply.send(await upsertPack({ id: req.params.id, ...req.body })));
...
app.get("/v1/jobs/:jobId", async (req, reply) => reply.send(await getJob(req.params.jobId)));
...
app.put("/v1/jobs/:jobId", async (req, reply) => {
    ...
    return reply.send(await updateJob(req.params.jobId, body));
});
app.get("/v1/traces", async (req, reply) => reply.send(await listTraces(req.query)));
app.post("/v1/integrations/mcp/register", async (req, reply) => {
    try {
        return reply.code(201).send(await registerThirdPartyMcp(req.body));
    }
    catch (e) {
        return reply.code(400).send({ error: e?.message ?? "invalid payload" });
    }
});
app.get("/v1/integrations/mcp", async (req, reply) => reply.send(await listThirdPartyMcps(req.query)));
app.get("/v1/integrations/mcp/:id", async (req, reply) => {
    const out = await getThirdPartyMcp(req.params.id);
    if (!out)
...
app.get("/v1/landing-zones/:zoneId", async (req, reply) => {
    try {
        return reply.send(await getLandingZone(req.params.zoneId));
    }
    catch (error) {
...
app.put("/v1/landing-zones/:zoneId", async (req, reply) => {
    ...
    try {
        return reply.send(await updateLandingZone(req.params.zoneId, body));
    }
    catch (error) {
        return reply.code(400).send({ error: error.message });
    }
...
app.delete("/v1/landing-zones/:zoneId", async (req, reply) => {
    try {
        return reply.send(await deleteLandingZone(req.params.zoneId));
    }
    catch (error) {
        return reply.code(400).send({ error: error.message });
    }
...
app.get("/v1/deployments", async (req, reply) => {
    const landingZoneId = req.query.landingZoneId;
    return reply.send(await listDeployments(landingZoneId));
});
app.get("/v1/deployments/:deploymentId", async (req, reply) => {
    try {
        return reply.send(await getDeployment(req.params.deploymentId));
    }
    catch (error) {
        return reply.code(404).send({ error: error.message });
    }
...
app.put("/v1/deployments/:deploymentId/status", async (req, reply) => {
    ...
    try {
        return reply.send(await updateDeploymentStatus(req.params.deploymentId, updates));
    }
    catch (error) {
        return reply.code(400).send({ error: error.message });
    }
...
app.delete("/v1/deployments/:deploymentId", async (req, reply) => {
    try {
        return reply.send(await deleteDeployment(req.params.deploymentId));
    }
    catch (error) {
        return reply.code(400).send({ error: error.message });
    }
...
app.post("/v1/deployments/:deploymentId/deploy", async (req, reply) => {
    try {
        await deployToKubernetes(req.params.deploymentId);
...
app.get("/v1/deployments/:deploymentId/status", async (req, reply) => {
    try {
        return reply.send(await getDeploymentStatus(req.params.deploymentId));
    }
    catch (error) {
        return reply.code(400).send({ error: error.message });
    }
...
app.put("/v1/deployments/:deploymentId/scale", async (req, reply) => {
    ...
    try {
        await scaleDeployment(req.params.deploymentId, body.replicas);
...
app.delete("/v1/deployments/:deploymentId/kubernetes", async (req, reply) => {
    try {
        await deleteKubernetesDeployment(req.params.deploymentId);
...
app.get("/v1/agents/:agentId", async (req, reply) => {
    try {
        return reply.send(await getAgent(req.params.agentId));
    }
    catch (error) {
        return reply.code(404).send({ error: error.message });
    }
...
app.put("/v1/agents/:agentId", async (req, reply) => {
    ...
    try {
        return reply.send(await updateAgent(req.params.agentId, body));
    }
    catch (error) {
        return reply.code(400).send({ error: error.message });
    }
...
app.post("/v1/agents/:agentId/regenerate-key", async (req, reply) => {
    try {
        const agent = await regenerateApiKey(req.params.agentId);
